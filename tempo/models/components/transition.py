...
class MBDTransitionPrior(nn.Module):
    ...
    def __init__(self, lags, latent_size, bias=False):
        ...
        self.L = lags
        self.transition = GroupLinearLayer(
            din=latent_size, dout=latent_size, num_blocks=lags, diagonal=False
        )
        self.bias = bias
        if bias:
...
class NPTransitionPrior(nn.Module):
    ...
    def __init__(self, lags, latent_size, num_layers=3, hidden_dim=64):
        super().__init__()
        self.L = lags
        # self.init_hiddens = nn.Parameter(0.01 * torch.randn(lags, latent_size))
        gs = [
            NLayerLeakyMLP(
                in_features=lags * latent_size + 1,
                out_features=1,
    ...
    def forward(self, x, masks=None):
        ...
        residuals = residuals.reshape(batch_size, -1, input_dim)
        sum_log_abs_det_jacobian = torch.sum(
