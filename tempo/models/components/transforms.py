...
from . import utils
from .base import FlowSequential, GroupLinearLayer
...
class ComponentWiseSpline(components.Transform):
    ...
    def spline_op(self, x: torch.Tensor, **kwargs) -> Tuple[torch.Tensor, torch.Tensor]:
...
class AffineMBD(components.Transform):
    ...
    def __init__(self, input_size: int, lags: int, diagonal: bool, hidden=None) -> None:
    ...
    def inverse(self, u: torch.Tensor, y: torch.Tensor):
...
class AffineMBDStep(components.Transform):
    ...
    def __init__(
        self, din: int, dout: int, lags: int, diagonal: bool, hidden=None
    ) -> None:
        """Constructs MBD object
        ...
        self.L = lags
        self.wt_func = GroupLinearLayer(
            din=din, dout=dout, num_blocks=self.L, diagonal=diagonal, hidden=hidden
        )
