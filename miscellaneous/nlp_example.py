...
def training_function(config, args):
    ...
    accelerator = Accelerator(
        cpu=args.cpu,  # If set to True, ignores GPU
        mixed_precision=args.mixed_precision,
    )

    # Sample hyperparameters for learning rate, batch size, seed, etc.
    lr = config["lr"]
...
def main():
    parser = argparse.ArgumentParser(
        description="Simple example of training script using Hugging Face's Accelerator."
    )
    parser.add_argument(
        "--mixed_precision",
        type=str,
...
Multiple GPUs and single node or multiple GPUs and multiple nodes:
accelerate config
