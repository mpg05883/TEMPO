...
parser.add_argument(
    "--checkpoints", type=str, default="/l/users/defu.cao/checkpoints_multi_dataset/"
)
...
parser.add_argument(
    ...
    type=int,
    default=1,
    help="1: equal sampling, 0: dont do the equal sampling",
)
...
for ii in range(args.itr):
    ...
    for dataset_singe in args.datasets.split(","):
        ...
        train_data, train_loader = data_provider(args, "train")
        if (
            dataset_singe not in ["ETTh1", "ETTh2", "ILI", "exchange"]
            and args.equal == 1
        ):
            train_data = Subset(train_data, choice(len(train_data), min_sample_num))
        if (
            args.electri_multiplier > 1
            and args.equal == 1
            and dataset_singe in ["electricity"]
        ):
            train_data = Subset(
                train_data,
        ...
        if (
            args.traffic_multiplier > 1
            and args.equal == 1
            and dataset_singe in ["traffic"]
        ):
            train_data = Subset(
                train_data,
