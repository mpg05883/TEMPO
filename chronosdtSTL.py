...
def read_dataset(dataset_name):
    ...
    try:
        ds = datasets.load_dataset(
            "autogluon/chronos_datasets", dataset_name, split="train"
        )
        ds.set_format("numpy")
        return ds
...
def create_sliding_windows(dataset_name, ds, window_size=432, x_size=336, y_size=96):
    ...
    for entry in ds:
        ...
        for i in range(0, len(targets) - window_size + 1):
            ...
            x = {
                "timestamp": window_timestamps[:x_size],
                "target": window_targets[:x_size],
            }
            y = {
                "timestamp": window_timestamps[x_size:],
                "target": window_targets[x_size:],
            ...
            windows_list.append(
                {
                    ...
                    "x_trend": np.zeros(x_size),
                    "x_seasonal": np.zeros(x_size),
                    "x_resid": np.zeros(x_size),
                    "y_trend": np.zeros(y_size),
                    "y_seasonal": np.zeros(y_size),
                    "y_resid": np.zeros(y_size),
                    "mean": 0,
                    "std": 0,
                }
            )
