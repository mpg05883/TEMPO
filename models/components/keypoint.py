...
class SpatialSoftmax(torch.nn.Module):
    ...
    def forward(self, feature):
        ...
        expected_x = torch.sum(
            Variable(self.pos_x) * softmax_attention, dim=1, keepdim=True
        )
        expected_y = torch.sum(
            Variable(self.pos_y) * softmax_attention, dim=1, keepdim=True
        )
...
class KeyPointNet(nn.Module):
    def __init__(
        ...
        self.keypoint_predictor = KeyPointPredictor(
            k, width=width, height=height, nf=nf, norm_layer=norm_layer, lim=lim
        )
    ...
    def keypoint_to_heatmap(self, keypoint, inv_std=10.0):
    ...
    def original_transport(
        self, src_feat, des_feat, src_hmap, des_hmap, des_feat_hmap=None
    ):
        ...
        src_digged = src_feat * (1.0 - src_hmap) * (1.0 - des_hmap)
    ...
    def transport(
        self, src_feat, des_feat, src_hmap, des_hmap=None, des_feat_hmap=None
    ):
        # src_feat: B x (nf * 4) x (H / 4) x (W / 4)
        # des_feat: B x (nf * 4) x (H / 4) x (W / 4)
        # src_hmap: B x n_kp x (H / 4) x (W / 4)
    ...
    def encode(self, xt):
        ...
        feat = feat.reshape(
            batch_size, length, feat.shape[1], feat.shape[2], feat.shape[3]
        )
